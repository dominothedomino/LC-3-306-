...
; The two data are read into registers R1 and R2. 
    LD  R0, PTR 
    LDR R1, R0, #0      ;R1 gets loaded with memory of x7000
    LDR R2, R0, #5      ;R2 gets loaded with memory of x7005
...
; The sum will be loaded in R3[15:8].
    ...
    LD R4, UNSNCLR  ;loading xFF00 into R4
    AND R1, R1, R4  ;clearing out the bits [7:0] in number 1
    AND R2, R2, R4  ;clearing out the bits [7:0] in number 2
    ADD R3, R1, R2  ;registers their sum in register R3
...
DONE    
    LD  R0, PTR 
    LDR R1, R0, #0      ;R1 gets reloaded with memory of x7000
    LDR R2, R0, #5      ;R2 gets reloaded with memory of x7005
    LD R4, TWOSCLR      ;R4 loaded with x00FF
    AND R1, R1, R4      ;utilizing R4, number 1's bits [15:8] are cleared
    AND R2, R2, R4      ;utilizing R4, number 2's bits [15:8] are cleared
    ADD R3, R1, R2
...
; If there was overflow, then store xBADD in x700A.
    ...
    LD R4, BK5MK        ;R4 loaded with bitmask x0100
    AND R5, R4, R3      ;R5 will be x0100 if there is overflow and x0 if not
    BRZ UNSNOOV         ;if R5 is x0, there is no overflow
    LD R4, OVERFLOWVAL  ;if R5 isn't x0, the overflow value is written to memory in x700A
    STR R4, R0, xA
    BR FINISH
